...
export function messageForIssue(code) {
...
export function toGrayscale(frame) {
...
export function computeLuminanceStats(frame) {
 ...
 */
export function computeSharpness(gray, width, height) {
 ...
 */
export function computeApproachProgress(faceHeightRatio, minFaceHeightRatio = DEFAULT_THRESHOLDS.minFaceHeightRatio, floor = APPROACH_FLOOR_RATIO) {
    if (faceHeightRatio == null || !Number.isFinite(faceHeightRatio))
        return 0;
    if (faceHeightRatio <= 0 || floor <= 0 || minFaceHeightRatio <= 0)
        return 0;
    ...
    if (faceHeightRatio <= floor)
        return 0;
    if (faceHeightRatio >= minFaceHeightRatio)
        return 1;
    const progress = Math.log(faceHeightRatio / floor) / Math.log(minFaceHeightRatio / floor);
    return Math.min(1, Math.max(0, progress));
}
export function computeMetrics(frame) {
 ...
 */
export function analyzeFrameQuality(input, thresholds = DEFAULT_THRESHOLDS) {
    ...
    else if (metrics.meanLuminance > thresholds.maxMeanLuminance ||
