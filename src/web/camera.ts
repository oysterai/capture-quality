...
export function buildCameraConstraints(facingMode: FacingMode = 'user'): MediaStreamConstraints {
  return {
    video: {
...
export function openCamera(facingMode: FacingMode = 'user'): Promise<MediaStream> {
 ...
 */
export async function attachStream(el: HTMLVideoElement, stream: MediaStream | null): Promise<void> {
  el.srcObject = stream
