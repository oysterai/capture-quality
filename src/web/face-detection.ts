...
let detectorOptions: TinyFaceDetectorOptions | null = null
let detectorScoreThreshold: number | null = null

function optionsFor(scoreThreshold: number): TinyFaceDetectorOptions {
 ...
 */
export async function loadFaceDetectorModels(modelUri: string = DEFAULT_MODEL_URI): Promise<boolean> {
  try {
...
const NO_FACE: FaceMeasurement = { detected: false, faceHeightRatio: null, score: null }
...
export async function measureFace(
  ...
  if (!video) return NO_FACE

  try {
    const detections = await detectAllFaces(
      video,
      optionsFor(options.scoreThreshold ?? DEFAULT_FACE_SCORE_THRESHOLD),
    )
